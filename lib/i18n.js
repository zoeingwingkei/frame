// Modified base on https://github.com/ppoffice/hexo-theme-minos/blob/master/lib/i18n.js
...
const post_path = require('hexo/lib/plugins/tag/post_path');
const {
    pathJoin,
    isDefaultLanguage,
    url_for,
...
/**
 * Remove 'undefined/' from permalink
 */
hexo.extend.filter.register('post_permalink', function (path) {
    // Maybe a bug in Hexo，if you put ':lang' in config.permalink, the default language will give you the 'undefined/' string，we will strip that in here.
 ...
 */
hexo.extend.helper.register('i18n_path', function (language, path) {
    let pagePathWithoutLang = (path || this.page.path).replace(`${this.page.lang}/`, '');
    return !isDefaultLanguage(language) ? pathJoin(language, pagePathWithoutLang) : pagePathWithoutLang;
});
...
/**
 * Get the tag path with lang slug
 */
 hexo.extend.helper.register('get_tag_path', function (tag) {
    return !isDefaultLanguage(this.page.lang) ? pathJoin(this.page.lang, tag.path) : tag.path
});

/**
 * Get the post path with lang slug
 */
hexo.extend.helper.register('root_path', function () {
...
hexo.extend.helper.register('alternative_lang_exist', function (alternativeLang) {
    if (routeList.length === 0) {
        routeList = hexo.route.list().map(x => x.replace('index.html', ''))
    }

    let pagePathWithoutLang = this.page.path.replace(`${this.page.lang}/`, '');
    let alternativeLangPath = !isDefaultLanguage(alternativeLang) ? pathJoin(alternativeLang, pagePathWithoutLang) : pagePathWithoutLang;
    let alternativeLangPathWithoutSuffic = alternativeLangPath.replace('index.html', '')

    return routeList.includes(alternativeLangPath) || routeList.includes(alternativeLangPathWithoutSuffic)
});
